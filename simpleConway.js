class SimpleConway {
	constructor (width, height, emptyVal) {
		this.width = width;
		this.height = height;
		this.birthVal = emptyVal + 1;
		this.killVal = emptyVal;

		this.prevGen = new Grid(width, height, emptyVal);
		this.currGen = new Grid(width, height, emptyVal);
	}
	...
	numLivingNeighbors(x, y) {
		let numLiving = 0;
		for (let i = 0; i < SimpleConway.#neighborDeltas.length; ++i) {
			const delta = SimpleConway.#neighborDeltas[i];
